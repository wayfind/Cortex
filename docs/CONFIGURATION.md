...
Cortex 支持以下配置方式（优先级从高到低）：

1. **环境变量**：推荐方式，适合所有部署场景
2. **`.env` 文件**：环境变量文件，适合本地开发和生产部署
3. **YAML 配置文件**（已弃用）：仍然支持但不推荐
4. **默认值**：内置的合理默认值

### 环境变量优先级

```bash
# 显式环境变量 > .env 文件 > config.yaml（已弃用）> 默认值
export CORTEX_LOG_LEVEL=DEBUG  # 最高优先级
```

> **注意**: 推荐使用 `.env` 文件进行配置。YAML 配置文件虽然仍被支持，但已不推荐使用，未来版本可能会移除。
